export default {
    ...
    users: {
      ...
      createApiKey: '新建 Key',
      createApiKeyHint: '为该用户新建 API 密钥',
      editApiKey: '编辑 API 密钥',
      editApiKeyHint: '编辑该 API 密钥的额度和限制',
      saveApiKeyHint: '保存 API 密钥设置',
      changeApiKeyGroupHint: '修改该 API 密钥绑定的分组',
      apiKeyName: 'Key 名称',
      apiKeyNameRequired: '请输入 Key 名称',
      customApiKey: '自定义 Key',
      customApiKeyPlaceholder: '留空则自动生成',
      customApiKeyHint: '可选。用于迁移或给外部系统预置固定 Key。',
      apiKeyStatus: '状态',
      apiKeyStatusActive: '启用',
      apiKeyStatusDisabled: '禁用',
      apiKeyStatusQuotaExhausted: '额度耗尽',
      apiKeyStatusExpired: '已过期',
      apiKeyQuota: '总额度',
      apiKeyQuotaUsed: '已用总额度',
      extraQuota: '额外限速额度',
      extraQuotaUsed: '已用额外限速额度',
      extraQuotaHint: '普通 5h/日/周限额用完后才消耗；仍受 Key 过期时间限制。',
      apiKeyExpiresAt: '过期时间',
      apiKeyExpiresAtHint: '留空表示不过期；设置后 extraQuota 也随 Key 一起失效。',
      rateLimitSection: '限速窗口',
      rateLimitHint: '0 表示不限额；请求会先消耗普通窗口额度，再消耗额外限速额度。',
      rateLimit5h: '5 小时限额',
      rateLimit1d: '日限额',
      rateLimit7d: '7 天限额',
      resetQuotaUsed: '同时重置已用总额度',
      resetExtraQuotaUsed: '同时重置已用额外限速额度',
      resetRateLimitUsage: '同时重置 5h/日/周已用量',
      zeroUnlimitedHint: '填 0 表示不限额',
      unlimited: '不限',
      extraQuotaDisabled: '未启用',
      neverExpires: '永不过期',
      nonNegativeAmountRequired: '额度和限额必须为 0 或正数',
      invalidExpiration: '请选择有效的过期时间',
      apiKeyCreated: 'API 密钥已创建',
      apiKeySaved: 'API 密钥已保存',
      apiKeySavedWithGrant: 'API 密钥已保存，已自动为用户添加「{group}」分组权限',
